...
- `AUTO_PLAYLISTS_RECENT_FROM_PLAYLISTS` - автоплейлисты из последних N треков указанных плейлистов (см. раздел «Последние N треков плейлиста» ниже).
...
## Последние N треков плейлиста

Переменная `AUTO_PLAYLISTS_RECENT_FROM_PLAYLISTS` создаёт автоплейлисты из последних N треков указанного плейлиста - по аналогии с recent-плейлистами из сохранённого, но источником служит конкретный плейлист.
...
- «Последние N треков» - треки, добавленные в плейлист позже всех (по дате добавления, свежие сверху). Повторяющиеся треки остаются только один раз.
...
- **сборный** - сборные плейлисты и плейлисты «последние N треков плейлиста», обновляется вместе с редким по `AUTO_PLAYLISTS_RARE_SYNC_INTERVAL_MS`.
